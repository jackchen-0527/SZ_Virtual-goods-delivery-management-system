<template>
    <el-main>
        ...
        <el-pagination background layout="prev, pager, next" :page-size="pageSize" :total="totalUsers"
            @current-change="getSalePersonalRequireTaskList" />
...
//页面初始化
onMounted(() => {
    getSalePersonalRequireTaskList()
})
...
const getSalePersonalRequireTaskList = async () => {
    try {
        const response = await axios.get(`/admin/sale_person_tasks/list`, { params: page })
        if (response.status === 200 && response.data.status === 200) {
            tableData.value = response.data.data
            page.total = response.data.total
            ElMessage.success('获取销售员需求列表成功')
        } else {
            ElMessage.error('获取销售员需求列表失败')
        }
    } catch (error) {
        console.error('获取列表出错:', error)
        ElMessage.error('获取列表失败，请检查网络')
